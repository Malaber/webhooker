...
- `webhooker_image_tag`: image tag for both services. Default: `{{ webhooker_collection_version }}`
- `webhooker_image`: container image for both services. Default: `ghcr.io/malaber/webhooker/webhooker:{{ webhooker_image_tag }}`
...
- `webhooker_worker_group_add`: supplemental groups added only to the worker container. When left empty, the role auto-detects the gid of `/var/run/docker.sock` on the target host and uses that. Default: `[]`
...
If the host Docker socket is not world-readable, the role auto-detects its gid and adds that gid to the worker container as a supplemental group. Set `webhooker_worker_group_add` explicitly only if you need to override the detected value.
...
```yaml
---
webhooker_image_tag: "0.3.21-rc.1" # Optional for RC testing before a collection RC is published.
